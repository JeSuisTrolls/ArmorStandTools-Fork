...
class ArmorStandCmdManager {
    ...
    private void getCommandsFromScoreboardTags() {
        ...
        for (String tag : tags) {
            if (tag.startsWith("ast-cmd-")) {
                ...
                if (command != null) addCommand(command, true);
            } else if (tag.startsWith("ascmd::v2::")) {
                ArmorStandCmd command = ArmorStandCmd.fromTag(tag);
                if (command != null) addCommand(command, false);
            }
        }
    }
    ...
    boolean hasCommands() { return !commands.isEmpty(); }

    void addCommand(ArmorStandCmd command, boolean saveToArmorStand) {
        commands.add(command);
        if (saveToArmorStand) command.saveTo(armorStand);
    }

    boolean removeCommand(int index) {
        ArmorStandCmd cmd;
        try { cmd = commands.get(index); }
    ...
    void executeCommands(Player p) {
        if (isOnCooldown()) {
            p.sendMessage(MM.parse(Config.cmdOnCooldown));
            return;
        }
        setOnCooldown();
        ...
        for (int n = 0; n < commands.size(); n++) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    if (!asCmd.execute(p)) {
                        p.sendMessage(MM.parse("<red>" + Config.errorExecutingCmd + " #" + num));
                    }
                }
    ...
    private void setOnCooldown() {
        ...
        if (cooldownTime == -1) cooldownTime = Config.defaultASCmdCooldownTicks;
        if (cooldownTime < 1) return;
        armorStand.setMetadata(ON_COOLDOWN_TAG, new FixedMetadataValue(AST.plugin, true));
        new BukkitRunnable() {
            @Override
            public void run() { armorStand.removeMetadata(ON_COOLDOWN_TAG, AST.plugin); }
        }.runTaskLater(AST.plugin, cooldownTime);
    }

    private boolean isOnCooldown() { return armorStand.hasMetadata(ON_COOLDOWN_TAG); }

    void setCooldownTime(int cooldown) {
        if (armorStand == null) return;
        List<String> tags = new ArrayList<>();
        for (String tag : armorStand.getScoreboardTags()) {
            if (tag.startsWith("ast-cdn-")) tags.add(tag);
        }
        for (String tag : tags) armorStand.removeScoreboardTag(tag);
    ...
    int getCooldownTime() {
        ...
        for (String tag : armorStand.getScoreboardTags()) {
            if (tag.startsWith("ast-cdn-")) {
                ...
                if (split.length < 2 || split[1].isEmpty()) return -1;
                try { return Integer.parseInt(split[1]); }
