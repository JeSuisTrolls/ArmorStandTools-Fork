...
public class Utils {
    ...
    public static boolean hasDisabledSlots(ArmorStand as) {
    ...
    public static int disabledSlotsAsInteger(ArmorStand as) {
    ...
    public static boolean hasPermissionNode(Player p, String perm) {
        if ((p == null) || p.isOp()) {
            return true;
        }
    ...
    public static void title(Player p, String msg) {
        p.showTitle(Title.title(
                Component.empty(),
                MM.parse(msg),
                Title.Times.times(java.time.Duration.ZERO, java.time.Duration.ofMillis(3500), java.time.Duration.ZERO)
        ));
    }
    ...
    private static String getItemStackTags(ItemStack is) {
        if (is == null) {
            return "";
        }
    ...
    private static int getItemCustomModelData(ItemStack is) {
    ...
    private static String itemInfo(ItemStack is) {
        ...
        int n = 0;
        if (!itemStackTags.isEmpty() || durability > 0) {
            sb.append(",tag:{");
            if (durability > 0) {
    ...
    private static String armorItems(EntityEquipment e) {
    ...
    private static String handItems(EntityEquipment e) {
    ...
    private static String angleInfo(EulerAngle ea) {
        return "[" + degrees(ea.getX()) + "f," + degrees(ea.getY()) + "f," + degrees(ea.getZ()) + "f]";
    }

    private static String pose(ArmorStand as) {
        return "Pose:{"
    ...
    public static String createEntityTag(ArmorStand as) {
        ...
        if (as.customName() != null)         sb.append("CustomName:'").append(GsonComponentSerializer.gson().serialize(as.customName())).append("',");
