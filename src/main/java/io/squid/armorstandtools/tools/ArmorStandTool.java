package io.squid.armorstandtools.tools;

import io.squid.armorstandtools.AST;
import io.squid.armorstandtools.config.Config;
import io.squid.armorstandtools.config.GuiItemConfig;
import io.squid.armorstandtools.utils.MM;
import io.squid.armorstandtools.utils.Utils;
...
public enum ArmorStandTool {
    ...
    public void setEnabled(FileConfiguration config) {
    ...
    public void use(Player p, ArmorStand as) {
    ...
    public static boolean isTool(ItemStack is) { return get(is) != null; }

    public static boolean isHoldingTool(Player p) {
    ...
    public ItemStack updateLore(ArmorStand as) {
        switch (this) {
            case INVIS:
                return setLoreFromTemplate(item, as.isVisible() ? "<green>" + Config.isTrue : "<red>" + Config.isFalse);
            case SIZE:
                return setLoreFromTemplate(item, as.isSmall() ? "<blue>" + Config.small : "<green>" + Config.normal);
            case BASE:
                return setLoreFromTemplate(item, as.hasBasePlate() ? "<green>" + Config.isOn : "<red>" + Config.isOff);
            case GRAV:
                return setLoreFromTemplate(item, as.hasGravity() ? "<green>" + Config.isOn : "<red>" + Config.isOff);
            case ARMS:
                return setLoreFromTemplate(item, as.hasArms() ? "<green>" + Config.isOn : "<red>" + Config.isOff);
            case INVUL:
                return setLoreFromTemplate(item, as.isInvulnerable() ? "<green>" + Config.isOn : "<red>" + Config.isOff);
            case SLOTS:
                return setLoreFromTemplate(item, Utils.hasDisabledSlots(as) ? "<green>" + Config.locked : "<red>" + Config.unLocked);
            case NAME:
                Component cn = as.customName();
                String cnStr = cn != null ? MM.serialize(cn) : "<blue>" + Config.none;
                return setLoreFromTemplate(item, cnStr);
            case GLOW:
                return setLoreFromTemplate(item, as.isGlowing() ? "<green>" + Config.isOn : "<red>" + Config.isOff);
            default:
                return item;
        }
    }
    ...
    private ItemStack setLoreComponents(ItemStack is, Component... components) {
        ...
        if (meta != null) {
            ...
            for (Component c : components) lore.add(Component.empty().decoration(net.kyori.adventure.text.format.TextDecoration.ITALIC, false).append(c));
            meta.lore(lore);
            is.setItemMeta(meta);
        }
        return is;
    ...
    private ItemStack setLore(ItemStack is, String... loreStrings) {
        ...
        if (meta != null) {
            List<Component> lore = new ArrayList<>();
            for (String s : loreStrings) lore.add(Component.empty().decoration(net.kyori.adventure.text.format.TextDecoration.ITALIC, false).append(MM.parse(s)));
            meta.lore(lore);
            is.setItemMeta(meta);
        }
        return is;
    ...
    private ItemStack setLoreFromTemplate(ItemStack is, String value) {
        if (loreTemplate.isEmpty()) return is;
        String[] resolved = loreTemplate.stream()
                .map(s -> s.replace("{value}", value))
                .toArray(String[]::new);
        return setLore(is, resolved);
    }

    public static ArmorStandTool get(Player p) { return get(p.getInventory().getItemInMainHand()); }

    public static ArmorStandTool get(ItemStack is) {
